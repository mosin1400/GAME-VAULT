...
function createCommunityStore({ file }) {
  ...
  async function read() {
    ...
    catch { return { ratings: {}, comments: [], events: [] }; }
  }
  ...
  async function event(game, type) { const data = await read(), now = new Date(); data.events.push({ game, type: type === 'download' ? 'download' : 'play', at: now.toISOString(), day: dayKey(now) }); data.events = data.events.slice(-5000); await write(data); }
  ...
  return { rate, rating, comment, comments, allComments, reply, remove, event, activity, weekly, exportData };
}
